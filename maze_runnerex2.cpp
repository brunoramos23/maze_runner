...
void print_maze() {
    const char path_char = 'o'; // Caractere para posição atual
    const char explored_char = '.'; // Caractere para posições exploradas
    const char wall_char = '#'; // Caractere para paredes
    const char exit_char = 's'; // Caractere para a saída

    for (int i = 0; i < num_rows; ++i) {
        for (int j = 0; j < num_cols; ++j) {
            char current_char = maze[i][j];
            
            if (current_char == path_char || current_char == explored_char) {
                printf("\33[1;31m"); // Define a cor para posição atual e posições exploradas
            } else if (current_char == wall_char) {
                printf("\33[1;30m"); // Define a cor para as paredes
            } else if (current_char == exit_char) {
                // Você pode adicionar outra cor se desejar para a saída
                printf("\33[1;32m"); // Por exemplo, verde para a saída
            }
